...
export class RedisService implements OnModuleDestroy {
  ...
  async rotateRefreshToken(
    ...
    oldKey: string,
    oldTtlSeconds: number
  ): Promise<void> {
    try {
      const pipeline = this.client.pipeline();
      pipeline.set(newKey, '1', 'EX', newTtlSeconds);
      pipeline.expire(oldKey, oldTtlSeconds);
      await pipeline.exec();
    } catch (error) {
      this.logger.error(`Redis rotateRefreshToken error: ${error instanceof Error ? error.message : 'Unknown error'}`);
      throw new Error('Cache operation failed');
    }
  }
