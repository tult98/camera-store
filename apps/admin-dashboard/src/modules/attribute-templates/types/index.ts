...
const attributeDefinitionSchema = z.object({
  label: z.string().min(1, 'Attribute name is required').max(100, 'Attribute name must be less than 100 characters'),
...
export const attributeTemplateSchema = z.object({
  ...
  is_active: z.boolean(),
  attribute_definitions: z.array(attributeDefinitionSchema).min(0, 'At least one attribute definition is recommended'),
});
