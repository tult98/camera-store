export const sanitizeHtml = (html: string): string => {
  ...
  const allowedVideoHosts = ['youtube.com', 'youtube-nocookie.com', 'youtu.be', 'vimeo.com', 'player.vimeo.com'];
  ...
  sanitized = sanitized.replace(/<iframe([^>]*)>(.*?)<\/iframe>/gi, (match, attrs) => {
    ...
    const isAllowed = allowedVideoHosts.some((host) => src.includes(host));
