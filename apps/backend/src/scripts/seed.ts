...
export default async function seedDemoData({ container }: ExecArgs) {
  ...
  // Update the API key with our consistent token
  const apiKeyModuleService = container.resolve(Modules.API_KEY)
  await apiKeyModuleService.updateApiKeys({
    id: publishableApiKey.id,
    token: 'pk_camera_store_dev_static_key_123456789',
  })
  ...
  logger.info('Publishable API Key: pk_camera_store_dev_static_key_123456789')
