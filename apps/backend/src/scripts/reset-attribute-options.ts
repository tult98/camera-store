...
export default async function resetAttributeOptions({ container }: ExecArgs) {
  ...
  for (const option of existingOptions) {
    await productAttributesService.deleteAttributeOptions(option.id)
  }
  ...
  const [newOptions] = await productAttributesService.listAndCountAttributeOptions()
  const groups = newOptions.reduce((acc: any, option: any) => {
