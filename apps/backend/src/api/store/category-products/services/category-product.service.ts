...
import type { CategoryProductsParams, Product, ProductProcessingContext } from '../types/category-products.types';
import { buildQueryFilters, buildSortOrder, buildGraphQuery } from './product-query-builder.service';

function attachProductAttributes(products: Product[]): Product[] {
  return products.map((product: Product) => {
    const { attribute_template_id: _attribute_template_id, ...attributeValues } = (product.metadata || {}) as Record<
      string,
      unknown
    >;
...
export async function getProductsByCategoryFn(container: MedusaContainer, params: CategoryProductsParams) {
  const { category_id, page, page_size, order_by, filters, search_query, region_id, currency_code, brand_id } = params;
  ...
  const queryFilters = buildQueryFilters(categoryIds, filters, productIdsByBrand);
  const sortOrder = buildSortOrder(order_by);
  const graphQuery = buildGraphQuery(queryFilters, sortOrder, context);
