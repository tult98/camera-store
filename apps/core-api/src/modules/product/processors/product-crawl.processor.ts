...
export class ProductCrawlProcessor extends WorkerHost {
  ...
  private renderMedia(media: MediaContent): string {
    if (media.type === 'image') {
  ...
  private renderFeature(feature: DescriptionFeature): string {
    ...
    const mediaHtml = feature.media ? this.renderMedia(feature.media) : '';

    const headerHtml = feature.header
      ? `<${headingTag} class='${headerClass}'>
  ${this.escapeHtml(feature.header)}
</${headingTag}>`
      : '';
  ...
  ${feature.subFeatures.map((sf) => this.renderFeature(sf)).join('\n')}
</div>`
        : '';
