...
export async function GET(req: MedusaRequest, res: MedusaResponse): Promise<void> {
  try {
    ...
    if (!regionId || !currencyCode) {
      ...
      logger.error(
        `Missing headers - region_id: ${regionId ? 'present' : 'missing'}, currency_code: ${
          currencyCode ? 'present' : 'missing'
        }`
      );
    ...
    const featuredCategories = categoriesResult.data.filter(
      (category) => category.metadata?.['hero_image_url'] && category.metadata?.['is_featured'] === true
    );
