...
export class AttributeFilter {
  static apply(products: Product[], filters: ApiFilters): FilterResult {
    ...
    const filteredProducts = products.filter((p: Product) => filteredProductIds.has(p.id));
  ...
  private static intersectFilterMatches(filterMatches: Map<string, Set<string>>): Set<string> {
