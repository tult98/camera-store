...
export const fetchBrands = async (
  ...
  const response = await sdk.client.fetch<FetchBrandsResponse>('/admin/brands', {
    method: 'GET',
    query: {
      q: query,
      limit,
...
export const fetchBrandById = async (id: string): Promise<Brand> => {
  const response = await sdk.client.fetch<{ brand: Brand }>(`/admin/brands/${id}`, {
    method: 'GET',
  });
...
export const createBrand = async (data: BrandSchemaType): Promise<Brand> => {
  const response = await sdk.client.fetch<{ brand: Brand }>('/admin/brands', {
    method: 'POST',
    body: data,
  });
...
export const updateBrand = async (id: string, data: BrandSchemaType): Promise<Brand> => {
  const response = await sdk.client.fetch<{ brand: Brand }>(`/admin/brands/${id}`, {
    method: 'PUT',
    body: data,
  });
