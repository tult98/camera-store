...
- **Advanced Filtering**: Implementing a robust filtering system based on product technical metadata and price.
...
- **Advanced Filtering is Standard**: For complex catalogs, allowing users to filter by key attributes is essential. This includes technical specifications and price to help users narrow down the product list effectively.
- **Guided Selling Boosts Confidence**: For high-consideration products, users respond well to "guided selling" through interactive quizzes and buying guides.
...
To ensure smooth development, the frontend will consume a well-defined API endpoint. Research indicates that Medusa's default Storefront API does not support the required advanced filtering (by price and metadata) out-of-the-box.

Therefore, the following endpoint must be created as a **custom Medusa API route**.
...
The frontend will use a predefined list of attributes (e.g., Brand, Sensor Size) to build the filtering interface.

See **Appendix A.2** for the detailed request/response structure.
...
- **Multi-Select**: Users can select multiple options within a single filter group (e.g., Brand: Sony, Canon).
- **URL Synchronization**: All active filters must be serialized into the URL for bookmarking and sharing.
- **Static Filter Groups**: The filter options will be based on a predefined set of product attributes rather than being dynamically generated.
...
*(Layouts from V1 are a strong foundation. The primary change is the addition of the list/grid view toggle and clear labeling for technical filters.)*
...
│ └─────────────────────┘│ ├────────────────────────────────────┤
│                        │ │ ┌─────┐ ┌─────┐ ┌─────┐ ┌─────┐  │
│ ▼ BRAND (TECHNICAL)    │ │ │ Prod│ │ Prod│ │ Prod│ │ Prod│  │
│ ☐ Canon                │ │ │  1  │ │  2  │ │  3  │ │  4  │  │
│ ☐ Nikon                │ │ └─────┘ └─────┘ └─────┘ └─────┘  │
│ ...                    │ │                                  │
│                        │ │ [← Previous] [1][2][3] [Next →] │
...
- **Phase 1: Foundation (Week 1)**: Project setup, layout components, basic navigation. **New Task**: Ensure the custom Medusa API endpoint (`/store/category-products`) is implemented and tested.
- **Phase 2: Core Filtering (Week 2)**: **Task**: Connect the frontend to the custom API. Implement the filtering UI based on technical metadata and price, and manage its state with URL synchronization.
...
- **Phase 5: Testing & Polish (Week 5)**: **New Task**: Add end-to-end tests for the custom API endpoint, filtering, and comparison user flows.
...
**Note**: This endpoint must be implemented as a **custom route** in the Medusa backend.
